...
export class UsuariosService {
  ...
  async modificar(dto: UpdateUsuarioDto) {
    ...
    // NOTA: Para solucionar lo que comentás de que cualquiera puede cambiar a cualquiera, 
    // el ID no debería venir del DTO del body, sino extraerse del Token JWT de la sesión en el Controller.
  ...
  async iniciarsesion(dto: LoginDto) {
    ...
    if (usuarioIngresado.contrasena !== dto.password) {
      ...
      if (nuevosIntentos === 3) {
        ...
        throw new BadRequestException('Contraseña incorrecta. La cuenta fue bloqueada y se le envio un email al correo asociado para desbloquearla');
      }
  ...
  async cerrarsesion(dto: LogoutDto) {
    ...
    // NOTA: Si manejás JWT con una lista negra o borrado de cookies en el cliente, 
    // la lógica principal del logout suele resolverse en el Controller o el cliente.
